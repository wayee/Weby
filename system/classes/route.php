...
class Route {
	...
	public static function get($name)
	{
		...
		{
			throw new Weby_Exception('The requested route does not exist: :route',
				array(':route' => $name));
		}
	...
	public static function cache($save = FALSE)
	{
		if ($save === TRUE)
		{
			...
			Weby::cache('Route::cache()', Route::$_routes);
		}
		else
		{
			if ($routes = Weby::cache('Route::cache()'))
			{
	...
	public function uri(array $params = NULL)
	{
		...
		while(preg_match('#'.Route::REGEX_KEY.'#', $uri, $match))
		{
			...
			if ( ! isset($params[$param]))
			{
				...
				throw new Weby_Exception('Required route parameter not passed: :param',
					array(':param' => $param));
			}
