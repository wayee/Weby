...
class Request {
	...
	public static function instance( & $uri = TRUE)
	{
		if ( ! Request::$instance)
		{
			...
			if ($uri === TRUE)
			{
				...
				else
				{
					...
					else
					{
						...
						throw new Weby_Exception('Unable to detect the URI using PATH_INFO, REQUEST_URI, or PHP_SELF');
					}

					// Get the path from the base URL, including the index file
					$base_url = parse_url(Weby::$base_url, PHP_URL_PATH);
					...
					if (Weby::$index_file AND strpos($uri, Weby::$index_file) === 0)
					{
						// Remove the index file from the URI
						$uri = substr($uri, strlen(Weby::$index_file));
					}
				}
			}
			...
			Request::$instance->headers['Content-Type'] = 'text/html; charset='.Weby::$charset;
		}
	...
	public static function user_agent($value)
	{
		...
		{
			// Load browsers
			$browsers = Weby::config('user_agents')->browser;
		...
		else
		{
			...
			$group = Weby::config('user_agents')->$value;
...
//				echo '<pre>';print_r($params);die();
		...
		throw new Weby_Exception('Unable to find a route to match the URI: :uri',
			array(':uri' => $uri));
	}
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new Weby_Exception('Download name must be provided for streaming files');
			}
...
//				$mime = File::mime($filename);
		...
		if ( ! empty($options['delete']))
		{
			...
			catch (Exception $e)
			{
				...
				if (is_object(Weby::$log))
				{
					// Add this exception to the log
					Weby::$log->add(Weby::ERROR, $error);
	...
	public function execute()
	{
		...
		try
		{
			...
			if ($class->isAbstract())
			{
				throw new Weby_Exception('Cannot create instances of abstract :controller',
	...
	public function generate_etag()
	{
		...
		{
			throw new Weby_Exception('No response yet associated with request - cannot auto generate resource ETag');
		}
